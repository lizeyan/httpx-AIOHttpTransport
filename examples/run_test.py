...
from httpx_faster_backends import (
...
async def benchmark_httpx_with_pycurl_transport(n):
    async with create_pycurl_backed_httpx_client() as client:
        # make sure code is right
        # print(await request_with_httpx(client))
        start = time.time()
        tasks = []
...
async def run_benchmark(requests=1000, rounds=10):
    ...
    for i in range(rounds):
        ...
        # httpx pycurl 测试
        await asyncio.sleep(1)
        httpx_time = await benchmark_httpx_with_pycurl_transport(requests)
        httpx_pycurl_times.append(httpx_time)
        print(f"httpx (pycurl transport) 耗时: {httpx_time:.2f} 秒")
    ...
    print(
        f"httpx (pycurl transport) 平均耗时: {statistics.mean(httpx_pycurl_times):.2f} 秒"
    )
